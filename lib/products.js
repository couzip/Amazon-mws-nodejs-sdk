...
module.exports = {
    ...
    GetLowestOfferListingsForSKU: function (config, params, xml, cb) {
        ...
        if (_.has(params, 'ExcludeMe') && (params['ExcludeMe'] === 'True' || params['ExcludeMe'] === 'False')) {
    ...
    /**
     * @absract  Returns the estimated fees for a list of products.
     * @description
        The GetMyFeesEstimate operation takes a list of products and marketplaces returns the fees for those products in those marketplaces.
        You can call GetMyFeesEstimate for a set of products before setting prices on those products.
        Your prices can then take estimated fees into account. You must specify your products by ASIN or SKU (not UPC, ISBN, etc).

        With each product fees request, you must include an original identifier.
        This identifier is included in the fees estimate so you can correlate a fees estimate with an original request.

        Note: The estimated fees returned by this API are not guaranteed. Actual fees may vary.
     * @param {Object} config Configuration file
     * @param {Object} params Required: FeesEstimateRequestList
     * @param {Boolean} xml flag. If true, returns raw xml although returns object
     * @param {Function} cb Callback function
     * @see http://docs.developer.amazonservices.com/en_US/products/Products_GetMyFeesEstimate.html
     */
     GetMyFeesEstimate: function (config, params, xml, cb) {
         if (typeof xml == 'function') {
             cb = xml;
             xml = false;
         } else if (typeof xml == 'undefined') {
         ...
         if (!_.has(params, 'FeesEstimateRequestList')) {
             cb({
                 "Error": "You need to pass parameter FeesEstimateRequestList"
             }, null);
             return;
         }

         var createParamsFromKey = function (key, values) {
             ...
             if (Object.prototype.toString.call(value) === '[object Object]') {
                 for (var keychild in value) {
                     if (value.hasOwnProperty(keychild)) {
                         counter++;

                         var newParams = createParamsFromKey(keychild, value);
                         if (newParams !== null) {
                             for (var newKey in newParams) {
                                 params[`${key}.${newKey}`] = newParams[newKey];
                             }
                         } else {
                             params[`${key}.${keychild}`] = value[keychild];
                         }
                     }
                 }
             }

             return counter > 0 ? params : null;
         };

         var parameters = { };
         for (var key in params['FeesEstimateRequestList']) {
             if (/FeesEstimateRequestList.FeesEstimateRequest.[1-9][0-9]*/.test(key)) {
                 parameters = Object.assign({ }, parameters, createParamsFromKey(key, params['FeesEstimateRequestList']));
             }
         }
